...
void render_font_8x8 ( unsigned char *fb, unsigned int tx, unsigned int ty, char *text, unsigned char colour ) {
  ...
  extram = fb;                             // start of RAM
  extram += ((unsigned int)ty)*FBWIDTH;    // skip down to target Y
  linestart = extram;
  ...
  while ( *text != '\0' ) {
    ...
    p = vincent_data [ (unsigned char) (*text) ];
    ...
    for ( y = 0; y < 8; y++ ) {
      ...
      extram += FBWIDTH; // go to next line of display

      p++;
    } // y

    extram -= (8*FBWIDTH); // rewind back to top-left corner
    extram += 8;
