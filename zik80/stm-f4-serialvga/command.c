    ...
    case SR:
      {
        ...
        USART_puts ( USART2, "+OK SR 25\n" );

        while ( srlen > 0 ) {
          c = queuepull();
          USART_puts ( USART2, "+REM 2\n" );

          if ( c ) {
            USART_puts ( USART2, "+REM 3\n" );

            if ( x + 30 > srw ) {
              USART_puts ( USART2, "+REM 4\n" );
              lamecopy ( fb_active + ( y * 256 ) + x, c, srw - x );
              y += 1;
              lamecopy ( fb_active + ( y * 256 ) + 0, c, 30 - ( srw - x ) );
              x = 30 - x;
            } else {
              USART_puts ( USART2, "+REM 5\n" );
              lamecopy ( fb_active + ( y * 256 ) + x, c, 30 );
              x += 30;
            }

            USART_puts ( USART2, "+OK SR 30\n" );
            srlen -= 30;
          }
